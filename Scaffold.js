...
if (
  "undefined" === typeof projectType ||
  undefined === packageJson.eri.repos[projectType]
) {
  ...
  console.log(
    `  ${chalk.cyan(program.name())} ${chalk.green("theme my-eri-project")}`
  );
  console.log();
  process.exit(1);
}
...
if ("undefined" === typeof directoryName || "" === directoryName) {
  ...
  console.log("For example:");
  console.log(
    `  ${chalk.cyan(program.name())} ${chalk.green("my-eri-project")}`
  );
  console.log();
  process.exit(1);
}
...
const directoriesToRename = [
  ...
  {
    from: "eri-plugin-scaffold",
    to: directoryName
  },
  {
    from: "languages/EriScaffold.pot",
...
clone(packageJson.eri.repos[projectType], "./" + directoryName, function(err) {
  if (err) {
    console.log(err);
  } else {
    ...
    directoriesToRename.forEach(function(dir) {
      if (fs.existsSync(directoryName + "/" + dir.from)) {
        fs.rename(
          directoryName + "/" + dir.from,
          directoryName + "/" + dir.to,
          function(err) {
            if (err) throw err;
